...
const ResultItem = React.forwardRef(({ action, active, currentRootActionId }, ref) => {
  ...
  return (
    <div ref={ref} className={`relative z-10 flex cursor-pointer items-center justify-between px-4 py-3`}>
      {active && <div id='kbar-result-item' className='border-sidebar-primary bg-sidebar-accent absolute inset-0 z-[-1]! border-l-4'></div>}
      ...
      {action.shortcut?.length ? (
        <div className='relative z-10 grid grid-flow-col gap-1'>
          {action.shortcut.map((sc, i) => (
            <kbd key={sc + i} className='flex items-center gap-1 rounded-md border px-1.5 py-1 text-xs font-medium shadow-sm'>
              {sc}
            </kbd>
          ))}
        </div>
