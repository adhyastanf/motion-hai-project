...
ALTER TABLE `projects` ADD CONSTRAINT `projects_status_id_project_statuses_id_fk` FOREIGN KEY (`status_id`) REFERENCES `project_statuses`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `tasks` ADD CONSTRAINT `tasks_assignee_id_members_id_fk` FOREIGN KEY (`assignee_id`) REFERENCES `members`(`id`) ON DELETE cascade ON UPDATE no action;